package com.epam.tat.test;
import org.apache.commons.lang3.RandomUtils;
import org.apache.commons.lang3.RandomStringUtils;

public class Constants {

    private Constants() {}

    public static final int VISIBILITY_TIMEOUT_SECONDS = 15;
    public static final int VALUE_TO_BE_TIMEOUT_SECONDS = 30;
    public static final int PAGE_LOAD_TIMEOUT_SECONDS = 30;
    public static final int FOLDER_NAME_NUMERIC_TILL = 999999999;
    public static final int MIN_CREDENTIALS_LENGTH = 8;
    public static final int MAX_CREDENTIALS_LENGTH = 20;
    public static final String LOGIN = "taf.alexander.gritsok";
    public static final String PASSWORD = "WebDriverGo";
}
